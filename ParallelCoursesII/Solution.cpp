...
class Solution {
  int minNumberOfSemesters(int n, std::vector<std::vector<int>>& dependencies,
    ...
    // prerequisites[i]: prerequisites mask of mask i, the set bits is
    // prerequisites
    std::vector<int> prerequisites(1 << n, 0);
    ...
    for (int i = 1; i < (1 << n); ++i) {
      ...
      // semester see: https://cp-algorithms.com/algebra/all-submasks.html
